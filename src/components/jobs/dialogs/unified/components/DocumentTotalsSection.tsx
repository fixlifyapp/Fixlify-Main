...
export const DocumentTotalsSection = ({
  ...
  // Compact mode - inline totals for side-by-side layout
  if (compact) {
    return (
      <div className="bg-white rounded-lg border border-gray-200 p-3">
        <div className="space-y-1.5 text-sm">
          ...
          <div className="flex justify-between items-center">
            <span className="text-gray-600">{taxLabel} ({effectiveTaxRate}%)</span>
            <span className="font-medium text-gray-900">{formatCurrency(tax)}</span>
          </div>
          <div className="border-t border-gray-200 pt-1.5">
            <div className={`flex justify-between items-center p-2 rounded-md ${totalBg}`}>
              <span className="text-sm font-bold text-gray-900">Total</span>
              <span className={`text-base font-bold ${totalColor}`}>
