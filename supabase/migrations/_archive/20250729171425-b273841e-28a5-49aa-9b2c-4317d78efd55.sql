-- Fix the execute_automation_for_record function to properly call the edge function
-- and ensure pg_net extension is enabled

-- Enable pg_net extension if not already enabled
CREATE EXTENSION IF NOT EXISTS pg_net;

-- Updated function to execute automation for a record
...
-- Updated job automation trigger function
...
BEGIN
  ...
  IF TG_OP = 'INSERT' THEN
    ...
    PERFORM execute_automation_for_record('job_created', trigger_data, org_id);
    PERFORM execute_automation_for_record('job_scheduled', trigger_data, org_id);
  END IF;
  ...
  IF TG_OP = 'UPDATE' AND OLD.status IS DISTINCT FROM NEW.status THEN
    ...
    PERFORM execute_automation_for_record('job_status_changed', trigger_data, org_id);
    
    -- Job completed specific trigger
    IF NEW.status = 'completed' OR NEW.status = 'Completed' THEN
      PERFORM execute_automation_for_record('job_completed', trigger_data, org_id);
    END IF;
  END IF;
