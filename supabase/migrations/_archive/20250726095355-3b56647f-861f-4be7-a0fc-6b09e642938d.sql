-- Simplified Database Schema Fixes for Jobs System
...
-- 4. Add missing our_price field to line_items table
...
-- 6. Add area_code column to phone_numbers table if missing
ALTER TABLE phone_numbers ADD COLUMN IF NOT EXISTS area_code TEXT;

-- 7. Create telnyx_calls table if it doesn't exist (referenced in dashboard widgets)
...
-- 8. Add RLS policies for telnyx_calls table
...
-- 9. Create conversations view as sms_conversations alias for backward compatibility
...
-- 10. Populate service field in jobs from job_types or default
UPDATE jobs 
