-- Fix the document numbering functions to avoid set-returning functions in CASE
...
-- Create a function to extract number from document number string
CREATE OR REPLACE FUNCTION public.extract_number_from_document(p_document_number text)
RETURNS integer
LANGUAGE plpgsql
IMMUTABLE
AS $$
...
BEGIN
  ...
  IF p_document_number IS NULL OR p_document_number = '' THEN
    RETURN 0;
  END IF;
  
  -- Extract numbers from the end of the string (after prefix and dash)
  v_matches := regexp_matches(p_document_number, '[0-9]+$');
  
  IF array_length(v_matches, 1) > 0 THEN
    BEGIN
      v_number := v_matches[1]::integer;
      RETURN v_number;
    EXCEPTION
      WHEN OTHERS THEN
        RETURN 0;
...
BEGIN
  ...
  IF p_document_type = 'invoice' THEN
    SELECT COALESCE(MAX(public.extract_number_from_document(invoice_number)), 0) 
    INTO v_result
    FROM invoices 
  ...
  IF p_document_type = 'estimate' THEN
    SELECT COALESCE(MAX(public.extract_number_from_document(estimate_number)), 0) 
    INTO v_result
    FROM estimates 
    WHERE user_id = p_user_id;
  END IF;
  ...
  IF p_document_type = 'payment' THEN
    BEGIN
      SELECT COALESCE(MAX(public.extract_number_from_document(payment_number)), 0) 
      INTO v_result
      FROM payments 
      WHERE user_id = p_user_id;
    EXCEPTION
