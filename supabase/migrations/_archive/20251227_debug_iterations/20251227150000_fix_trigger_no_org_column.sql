-- Fix trigger function for tables WITHOUT organization_id column
-- clients, estimates, invoices only have user_id
-- This migration updates the trigger to handle both cases
...
BEGIN
  ...
  -- If no organization_id, look up from user's default org
  IF v_organization_id IS NULL AND NEW.user_id IS NOT NULL THEN
    SELECT uo.organization_id INTO v_organization_id
  ...
  -- Still no org? Try from profiles table
  IF v_organization_id IS NULL AND NEW.user_id IS NOT NULL THEN
  ...
  IF v_organization_id IS NULL THEN
    RAISE NOTICE 'No organization_id found for table %, skipping automation', TG_TABLE_NAME;
    RETURN NEW;
  END IF;
  ...
  LOOP
    ...
    IF should_trigger THEN
      ...
      RAISE NOTICE 'Created automation log % for workflow % (org: %, table: %)', log_id, workflow_record.id, v_organization_id, TG_TABLE_NAME;
    END IF;
  END LOOP;
...
'Multi-tenant automation trigger - handles tables with AND without organization_id column. Falls back to user_organizations lookup.';
